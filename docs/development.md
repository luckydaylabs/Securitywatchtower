...
Checks use separate macOS, Windows, Linux, and AI research lanes. Active platforms dispatch concurrently, with independent verification before findings are published. There is no three-announcement cap: batches are sized to the actual request, and verification can split a large result into smaller requests. There is no fixed eight-run cap per check. Provider usage limits and failures can still leave announcements pending, with coverage reported as partial. Platforms without new evidence do not consume agent runs. Exact advisory URLs and captured evidence are supplied; orchestration, validation, routing, and persistence run in application code. The 9,500-character input guard is an application safeguard, not a documented Nimble limit.
