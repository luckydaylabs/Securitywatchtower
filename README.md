...
The monitor source policy is ordered around primary advisory feeds: Microsoft MSRC CVRF, Debian advisories and tracker data, Ubuntu Security Notices, Red Hat advisories and CSAF, SUSE CSAF, Alpine SecDB, Apple security releases and announcement archives, and Linux kernel CVE announcement archives. It also permits Fedora updates, OpenAI and Anthropic security disclosures, Google AI security disclosures, CISA KEV, NVD, OSV, and OWASP AI guidance when those sources directly support a finding. The route sends this allowlist and priority order to every Nimble pipeline stage and accepts output URLs only from the approved domains.
